...
export function buildApp() {
  ...
  app.get(
    ...
    async () => {
      return {
        ...
        description:
          "Independently verifies OneTask DeFi vault migration constraints and produces hash-bound risk evidence.",
        image: `${publicBaseUrl}/agent.svg`,
        services: [
          {
  ...
  app.get("/agent.svg", async (_request, reply) => {
    const svg = `
      <svg
    ...
    async (request, reply) => {
      ...
      if (!parsedInput.success) {
        return reply.code(400).send({
          code: "INVALID_RISK_EVALUATION_INPUT",
          message:
